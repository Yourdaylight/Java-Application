...
public class jianfanConvert {
    ...
    //简体转繁体
    public static void jian2Fan(String words){
        //遍历每个字，进行转换
    ...
    public static void fan2Jian(String words){
        //遍历每个字，进行转换
        for (int i = 0; i < words.length(); i++) {
            char tempChar = words.charAt(i);
            int position = fanti.indexOf(tempChar);//此方法调用时间复杂度为O(n)
            char fantiChar;
            if (position == -1) {
                fantiChar = tempChar;
            } else {
                fantiChar = jianti.charAt(position);
            }
            System.out.print(fantiChar);
        }
    }
